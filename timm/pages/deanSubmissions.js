$(function () {
  ...
  function filterSubmissions() {
    ...
    const filtered = submissionsData.filter(row => {
      ...
      const st = (row.approvalStatus || '').toLowerCase();
  ...
  function initializeActions() {
    ...
    $(document).off('click', '.approve-submission').on('click', '.approve-submission', function (e) {
      ...
      if (confirm('Approve this submission?')) updateSubmissionStatus(id, 'APPROVED');
    });
    ...
    $(document).off('click', '.reject-submission').on('click', '.reject-submission', function (e) {
      e.preventDefault();
      const id = $(this).data('id');
      if (confirm('Reject this submission?')) updateSubmissionStatus(id, 'DENIED');
    });
    ...
    $(document).off('click', '.review-submission').on('click', '.review-submission', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // CORRECT: Use res.file (not res.url)
  ...
  function updateSubmissionStatus(submissionId, status) {
    $.ajax({
      ...
      success: function (resp) {
        let result;
        try { result = typeof resp === 'string' ? JSON.parse(resp) : resp; }
        catch (e) { result = {}; }

        if (result.success) {
          alert(`Submission ${status === 'APPROVED' ? 'approved' : 'denied'}!`);
          loadDeanSubmissions();
        } else {
  ...
  function getStatusClass(status) {
    ...
    if (s === 'APPROVED')   return 'border-green-500 bg-green-100 text-green-700';
    if (s === 'PUBLISHED')  return 'border-blue-500 bg-blue-100 text-blue-700';
    if (s === 'DENIED' || s === 'REJECTED') return 'border-red-500 bg-red-100 text-red-700';
  ...
  function showError(msg) { $tbody.html(`<tr><td colspan="10" class="px-4 py-3 text-center text-sm text-red-600">${msg}</td></tr>`); }
  function showMessage(msg) { $tbody.html(`<tr><td colspan="10" class="px-4 py-3 text-center text-sm text-gray-500">${msg}</td></tr>`); }
});