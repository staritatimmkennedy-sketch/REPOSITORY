...
try {
    $stmt = $conn->prepare("CALL sp_updateMaterialType(:id, :name, :desc)");
    ...
    if ($result['rowcount'] > 0) {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Material type not found or no changes made'
        ]);
    }
    ...
    echo json_encode([
        'success' => false,
        'error'   => 'Failed to update material type: ' . $e->getMessage()
    ]);
}
?>