$(function () {
  ...
  function loadApprovedMaterials() {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        console.error("Error loading approved materials:", error, xhr.responseText);
        $tbody.html(`<tr><td colspan="10" class="px-4 py-3 text-center text-red-500">Error loading materials. Please try again.</td></tr>`);
      }
    });
  }
  ...
  function renderApprovedMaterials(data) {
    ...
    data.forEach(row => {
      ...
      // Show action buttons for all approved materials (similar to dean's pending logic)
      const actionButtons = `
  ...
  function initializeActions() {
    ...
    $(document).off('click', '.publish-submission').on('click', '.publish-submission', function(e) {
      ...
      $('.dropdown-menu').addClass('hidden'); // Close dropdown
    });
      ...
      success: function(response) {
        try {
          ...
          if (result.success) {
            ...
            setTimeout(() => {
              $publishModal.addClass('hidden');
              loadApprovedMaterials(); // Reload to remove published item
            }, 1500);
          } else {
        ...
        success: function(response) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    loadApprovedMaterials(); // Reload the data
                } else {
